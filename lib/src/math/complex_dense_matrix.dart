...
/// Class representing a matrix of [Complex] values in [ComplexDenseMatrix.rows] rows and [ComplexDenseMatrix.columns] columns
class ComplexDenseMatrix extends ComplexMatrix {
  ...
  factory ComplexDenseMatrix(List<List<Complex>> values) {
    ...
    return ComplexDenseMatrix.generate(rows, columns, (r, c) => values[r][c]);
  }
  ...
  ComplexDenseMatrix.identity(int rows)
    : this.generate(
        rows,
        rows,
  ...
  @override
  ComplexDenseMatrix clone() =>
      ComplexDenseMatrix.fromComplexArray(rows, columns, _values);

  @override
  int get memoryFootprint => _values.memoryFootprint;
  ...
  void set(int row, int column, Complex value) =>
      _values.set(_idx(row, column), value);
  ...
  @override
  ComplexDenseMatrix copy(ComplexMatrix other) {
  ...
  @override
  ComplexDenseMatrix operator -() => clone().neg();

  /// Negates all values in this instance
  @override
  ComplexDenseMatrix neg() {
  ...
  @override
  ComplexDenseMatrix operator +(ComplexDenseMatrix other) => clone().add(other);

  /// Adds values of [other] to the values in this instance
  @override
  ComplexDenseMatrix add(ComplexDenseMatrix other) {
    if (rows != other.rows || columns != other.columns) {
      throw InvalidOperationException(
  ...
  @override
  ComplexDenseMatrix operator -(ComplexDenseMatrix other) => clone().sub(other);

  /// Subtracts values of [other] from the values in this instance
  @override
  ComplexDenseMatrix sub(ComplexDenseMatrix other) {
    if (rows != other.rows || columns != other.columns) {
      throw InvalidOperationException();
    }
    final ov = other._values;
  ...
  /// [other] may be a [num], a [Complex] or another [ComplexDenseMatrix]
  /// if [other] is a [ComplexDenseMatrix], this instance and [other] must be square matrices of the same size
  @override
  ComplexDenseMatrix mul(Object other) {
    if (other is num || other is Complex) {
      _values.scale(other);
    } else if (other is ComplexDenseMatrix) {
  ...
  /// [other] may be a [num], a [Complex] or another [ComplexDenseMatrix]
  /// if [other] is a [num] or [Complex], the resulting matrix will have the same size
  /// if [other] is a [ComplexDenseMatrix], [other] must be invertible and the resuting matrix is obtained by multipltying this instance by the [inverse] of [other]
  @override
  ComplexDenseMatrix operator /(Object other) {
    if (other is num || other is Complex) {
  ...
  /// [other] may be a [num], a [Complex] or another [ComplexDenseMatrix]
  /// if [other] is a [ComplexDenseMatrix], the result is obtained by calling [mul] with the [inverse] of [other]
  @override
  ComplexDenseMatrix div(Object other) {
    if (other is num || other is Complex) {
  ...
  /// Returns true if [other] is a [ComplexDenseMatrix] of the same size, and all of their values are equal down to a precision of [precision]
  @override
  bool equals(ComplexMatrix other, {double precision = 0}) {
    if (rows != other.rows || columns != other.columns) return false;
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < columns; c++) {
        if (!get(r, c).equals(other.get(r, c), precision: precision)) {
          return false;
        }
      }
    }
  ...
  /// The resulting matrix will have [columns] rows and [rows] columns
  @override
  ComplexDenseMatrix transpose() {
    final t = ComplexDenseMatrix.zero(columns, rows);
    final tv = t._values;
    final tidx = t._idx;
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < columns; c++) {
  ...
  /// This instance must be a square matrix
  /// if [rows] == [columns] == 1, returns a [ComplexDenseMatrix] holding a single value which is the inverse of the value in this instance
  /// if [rows] == [columns] > 1, the inverse is obtained by Gaussian elimination
  @override
  ComplexDenseMatrix inverse() {
    if (!isSquare) {
      throw InvalidDimensionsException();
    }
    ...
    if (rows == 1) {
      ...
      final copy = ComplexDenseMatrix.zero(rows, rows * 2);
      final identity = ComplexDenseMatrix.identity(rows);
  ...
  static ComplexDenseMatrix deserialize(int rows, int columns, List payload) =>
      ComplexDenseMatrix.generate(rows, columns, (r, c) {
        final value = payload[r * columns + c];
        return Complex(re: value[0], im: value[1]);
      });
}
